...
"""
        ...
        # Split Image section (inline)
        split_count = d.get("split_segment_count", 1)
        split_size = d.get("split_size", 0)
        if split_count > 1 and split_size > 0:
            from fx.core.acquisition.split_writer import format_split_size
            content += f"Split Image      : YES — {split_count} segment(s), {format_split_size(split_size)} each\n"
            split_paths = d.get("split_segment_paths", [])
            if split_paths:
                content += "Segment Files    :\n"
                for sp in split_paths:
                    content += f"  {os.path.basename(sp)}\n"
        else:
            content += "Split Image      : No (single file)\n"
    ...
    def _generate_pdf(d: dict, summary: str, filepath: str) -> None:
        ...
        with warnings.catch_warnings():
            ...
            if split_count > 1 and split_size > 0:
                from fx.core.acquisition.split_writer import format_split_size
                pdf.set_font("helvetica", "", 11)
                row("Split Image:", f"YES - {split_count} segment(s), {format_split_size(split_size)} each")
                split_paths = d.get("split_segment_paths", [])
                if split_paths:
                    pdf.set_font("courier", "", 9)
                    for sp in split_paths:
                        pdf.cell(0, 6, f"  {os.path.basename(sp)}", ln=1)
            else:
                pdf.set_font("helvetica", "", 11)
                row("Split Image:", "No (single file)")
