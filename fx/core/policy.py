...
def apply_write_blocker(ssh: paramiko.SSHClient, disk: str) -> None:
    ...
    _, err, code = ssh_exec(ssh, f"sudo -n blockdev --setro {disk}")
    if code != 0:
        raise RuntimeError(f"Write-blocker failed (blockdev --setro). {err}")

    # 2) Validate read-only flag
    out, err, code = ssh_exec(ssh, f"sudo -n blockdev --getro {disk}")
    if code != 0:
    ...
    ssh_exec(ssh, f"sudo hdparm -r1 {disk}")
...
def build_dd_command(disk: str, skip_bytes: int, safe_mode: bool) -> str:
    ...
    return f"sudo dd if={disk} bs=4M skip={skip_bytes} iflag=skip_bytes{conv_flag} status=none"
