...
* **Bandwidth Throttling:** Integrates with `pv` (Pipe Viewer) to limit transfer speeds and prevent network bottlenecks.
...
* `main_qt6.py`: Application entry point and UI launcher.
* `codes/gui.py`: Manages the PyQt6 interface, user interactions, and PDF reporting engine.
* `codes/acquisition.py`: Handles SSH tunneling, `dd` streaming, bandwidth throttling, and Triage execution.
...
5. Select the report language (EN/TR) from the popup menu after acquisition completes.
...
2. **Preservation:** Enforces Read-Only modes and secures images with Dual-Hash signatures (SHA-256 & MD5).
...
* **Dependencies:** `PyQt6`, `fpdf2`, `pv`

```bash
# Install system dependency for bandwidth throttling (Fedora)
sudo dnf install pv
