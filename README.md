...
* **Dynamic Real-Time Progress:** Features a live, zero-deadlock GUI progress bar powered by `pv` (Pipe Viewer), allowing examiners to track byte-by-byte acquisition streams.
...
* **Bandwidth Throttling:** Integrates with `pv` to limit transfer speeds and prevent network bottlenecks.
...
5. Choose the evidence format (**E01** or **RAW**) from the popup dialog.
6. Select the report language (EN/TR) from the popup menu after acquisition completes.
